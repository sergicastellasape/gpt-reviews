...
if not ARGS.date:
    ...
    today = datetime.strptime(ARGS.date, "%Y-%m-%d").strftime("%A, %B %d, %Y")
...
if ARGS.scope in ["scripts", "recordings", "all"]:
    ...
    for i, newspiece in enumerate(news):
        news_scripts.append(
            write(
                system_prompt=prompts["system_prompt_news_conversation"],
    ...
    transition_1 = write(
        system_prompt=prompts["system_prompt_reads"],
    ...
    for (
        ...
        paper_scripts.append(
            write(
                system_prompt=prompts["system_prompt_paper_conversation"],
...
if ARGS.scope in ["recordings", "all"]:
    intro_section = speak(
        ...
        speaker={"name": "en-US-DavisNeural", "style": "angry"},
    ...
    transition_1_section = speak_conversation(
        ...
        guest={"name": "en-US-SaraNeural", "style": "unfriendly"},
        delimiter={"host": "G: ", "guest": "O: "},
        paragraph_silence=200,
        sentence_silence=400,
    )

    transition_2_section = speak(
        script=transition_2,
    ...
    ad_section = speak_conversation(
        script=ad_script,
        audio_path="assets-today/audio-clips/ad.wav",
        host={"name": "en-US-NancyNeural", "style": "excited"},
        guest={"name": "en-US-TonyNeural", "style": "cheerful"},
    ...
    for i, script in enumerate(paper_scripts):
        paper_sections.append(
            speak_conversation(
                ...
                host={"name": "en-US-DavisNeural", "style": "chat"},
                guest={"name": "en-US-AriaNeural", "style": "chat"},
...
if ARGS.scope == "all":
    ...
    if reads_scripts:
        ...
        for i, read_section in enumerate(reads_sections):
            ...
            section = section.overlay(background, loop=True).fade_out(
                duration=100
            )
    ...
    ad_section = ad_section.overlay(ad_music, loop=True).fade_out(
        duration=4000
    )

    sponsor_section = fake_sponsor_jingle.append(
        ad_section, crossfade=1000
    ...
    outro_bg = outro_bg.append(outro_bg_drums * repetitions, crossfade=10)
